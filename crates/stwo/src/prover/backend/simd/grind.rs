...
use crate::core::channel::{
    Blake2sChannelGeneric, Poseidon31Channel, Sha256Channel, Sha256Poseidon31Channel,
};
...
use crate::prover::backend::simd::poseidon31::permute;
...
impl GrindOps<Sha256Channel> for SimdBackend {
    fn grind(channel: &Sha256Channel, pow_bits: u32) -> u64 {
        let _span = span!(Level::TRACE, "Simd Sha256 Grind", class = "Sha256 Grind");

        // TODO(first): support more than 32 bits.
        assert!(pow_bits <= 32, "pow_bits > 32 is not supported");
        let digest: Sha256Hash = channel.digest();

        #[cfg(not(feature = "parallel"))]
        let res = (0..)
            .find_map(|hi| grind_sha256(digest, hi, pow_bits))
            .expect("Grind failed to find a solution.");

        #[cfg(feature = "parallel")]
        let res = parallel_grind(digest, pow_bits, grind_sha256);

        res
    }
}

fn grind_sha256(digest: Sha256Hash, hi: u64, pow_bits: u32) -> Option<u64> {
...
impl GrindOps<Poseidon31Channel> for SimdBackend {
    fn grind(channel: &Poseidon31Channel, pow_bits: u32) -> u64 {
        ...
        #[cfg(feature = "parallel")]
        let res = parallel_grind(digest, pow_bits, grind_poseidon31);
...
mod tests {
    ...
    #[test]
    fn test_grind_sha256_is_deterministic() {
        test_grind_is_deterministic::<Sha256Channel>();
    }

    #[test]
    fn test_grind_poseidon31_is_deterministic() {
        test_grind_is_deterministic::<Poseidon31Channel>();
    }
