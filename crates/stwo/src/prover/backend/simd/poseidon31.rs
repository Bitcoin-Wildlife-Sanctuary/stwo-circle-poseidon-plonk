...
use crate::prover::backend::simd::m31::{PackedM31, LOG_N_LANES};
            ...
            let hash_column = if !columns.is_empty() {
                ...
                if num_chunk == 1 {
                    ...
                    if remain == 0 {
                        let mut state = [PackedM31::zero(); 16];
                        for j in 0..8 {
                            state[j] = columns[len - 8 + j].data[i];
                        }
                        for j in 0..8 {
                            state[j + 8] = digest[j];
                        }
                        ...
                        let mut state = [PackedM31::zero(); 16];
                        for j in 0..remain {
                            state[j] = columns[len - remain + j].data[i];
                        }
                        for j in 0..8 {
                            state[j + 8] = digest[j];
                        }
...
mod test {
    ...
    use crate::prover::backend::simd::poseidon31::{permute_get_capacity, permute_get_rate};
    use crate::prover::backend::simd::SimdBackend;
